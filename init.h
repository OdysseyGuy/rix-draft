 ...
 * 
 * @param _stage
 * Stage at which this hook would be registered.
 * 
 * @param _priority
 * Hook priority. Hooks with higher priority that are
 ...
 * 
 * @param _args
 * Pointer to the arguments passed to the hook function.
 */
#define REGISTER_INIT_HOOK(_name, _stage, _priority, _fn, _args)    \
    static __init_hook __aligned(8)                                 \
    init_stage_hook_t __init_hook_ ## _name = {                     \
