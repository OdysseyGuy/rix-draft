...
#define SWAPINIT(a, es)                                                        \
    swaptype = (((uint8_t *)a - (uint8_t *)0 | es) % sizeof(size_t)) ? 2       \
             : es > sizeof(size_t)                                   ? 1       \
                                                                     : 0
...
static void swapfunc(uint8_t *a, uint8_t *b, size_t n, int swaptype)
{
    if (swaptype <= 1) {
        size_t t;
        for (; n > 0;
             a += sizeof(size_t), b += sizeof(size_t), n -= sizeof(size_t)) {
 ...
 *
 * @param es Size of each element in the array.
 *
           ...
           int (*cmp)(const void *, const void *))
{
    ...
    if (n < 7) {
        for (pm = arr + es; pm < (uint8_t *)arr + (n * es); pm += es) {
            for (pl = pm; pl > (uint8_t *)arr && cmp(pl - es, pl) > 0;
                 pl -= es) {
                swap(pl, pl - es);
            }
        }
